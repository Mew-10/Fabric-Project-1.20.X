package net.terk.mccourse.item.custom;
...
public class MetalDetectorItem extends Item {
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        if (!context.getWorld().isClient){
            ...
            for(int i = 0; i <= posclick.getY() + 64; i++){
                BlockState blockState = context.getWorld().getBlockState(posclick.down(i));
                Block block = blockState.getBlock();

                if(isValuableBlock(blockState)){
                    outputValuableCoordinates(posclick.down(i), player, block);
                    foundblock = true;
